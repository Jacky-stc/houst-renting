...
const Search:FC<Props> = ({apiKey, sheetId}) => {
    ...
    const [searchStatus, setSearchStatus] = useState<SearchStatus>({status: 'default'})
  ...
  return (
    <div className='mx-6 tracking-wide w-11/12 font-inter sm:w-auto flex-1'>
        <div className={`flex items-center px-2 py-1 w-full ml-auto sm:w-72 rounded ${inputFocus?' border border-sky-400':'border'}`}>
            <IoSearchOutline></IoSearchOutline>
            <div className='flex justify-between w-full sm:w-auto'>
                <form action={"."} className='flex w-full' >
                    <input style={{WebkitTapHighlightColor: 'transparent', outline:'none'}} type='search' value={inputValue} onFocus={()=>{setInputFocus(true)}} onBlur={()=>{setInputFocus(false)}} onChange={e=> setInputValue(e.target.value)} onKeyDown={(e)=>{handleEnter(e)}} className="rounded-sm px-2 py-1 mx-1 w-full sm:w-auto focus:outline-0 outline-0"  placeholder="輸入物件編號或區域" spellCheck='false'></input>
                    <button onClick={handleSearch} className='hidden sm:block'>搜尋</button>
                </form>
            </div>
        </div>
        {(searchStatus.status === 'default' || searchStatus.status === 'no result') && <Card searchStatus = {searchStatus}></Card>}
