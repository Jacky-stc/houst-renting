...
const getFilterData = (inputValue: string, HouseListData: HouseListData[]) => {
  // 搜尋區域名
  if (regionArray.includes(inputValue) || regionArray.includes(inputValue + '區')) {
    const data = HouseListData.filter((data) => data.value[5] === inputValue || data.value[5] === inputValue + '區');
    return data;
  }
  // 搜尋地址
  const data = HouseListData.filter((item) => item.value[6].includes(inputValue));
  return data;
};
...
export const handleSearch = async (inputValue: string) => {
  ...
  if (Object.keys(regionList).includes(inputValue.slice(0, 1))) {
    const formatedData = HouseListData.map((item) => rentingDataFormat(item.value, item.index.toString()));
  ...
  if (/^\d+$/.test(inputValue)) {
    const targetObject = HouseListData.filter((item) => typeof item.value[21] === 'string' && item.value[21].includes(inputValue));
    if (targetObject.length > 1) {
      ...
      useRentingData.setState({
        rentingData: rentingDataFormat(targetObject[0].value, targetObject[0].index.toString()),
        searchStatus: 'result',
      });
      return;
    } else {
