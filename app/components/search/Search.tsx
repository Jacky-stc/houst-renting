...
const Search: FC = () => {
  ...
  return (
    <div className="relative overflow-hidden tracking-wide w-11/12 font-inter sm:w-auto flex-1">
      ...
      {pageNow === 'MyHouseList' && !personName ? <SearchName /> : <SearchInput />}

      {searchStatus !== 'result' && <Card></Card>}
      {rentingData && searchStatus === 'result' && <HouseInfo rentingData={rentingData} houseList={rentingList}></HouseInfo>}
      <div className={`mt-8 px-4`}>
        {sortedRentingList &&
          !rentingData &&
          sortedRentingList.map((houseObject) => (
