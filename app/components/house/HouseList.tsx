...
import { personList, rentingDataFormat } from '../../lib/utils';
...
const HouseList: React.FC<HouseListProps> = ({ houseObject, index }) => {
  ...
  const BGColor = personList.find((person) => person.name === houseInfo.業務編號)?.BGColor || 'gray';

  const handleClick = () => {
    useRentingData.setState({ rentingData: houseInfo, searchStatus: 'result' });
  };
  return (
    <div
    ...
    >
      <div className="flex flex-row">
        <div className="inline-block align-sub text-[1.35rem]">{houseInfo.編號}</div>
      ...
      <div className="flex flex-row items-center mt-[0.3rem]">
        ...
        >
          {houseInfo.業務編號.slice(houseInfo.業務編號.length - 1, houseInfo.業務編號.length)}
        </span>
        <span className="max-w-[75%] text-ellipsis text-[0.75rem]  whitespace-nowrap overflow-hidden">{houseInfo.地址}</span>
      </div>
